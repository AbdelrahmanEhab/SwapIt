...
function Search(props: SearchProps) {
    ...
    const {category, search, setSearch, setCategory} = props
                    ...
                    <div className="w-full flex flex-col justify-center items-center gap-10">
                        ...
                        <div className="flex md:flex-row flex-col md:gap-0 gap-5 items-center justify-center w-full">
                            <div className="h-10 md:w-fit w-full">
                                <select value={category} onChange={(e) => {setCategory(e.target.value); nav(`/home/${e.target.value}`)}} className="h-full w-full text-white bg-blue-800 px-4 py-2 focus:outline-1 focus:outline-blue-800 outline-1 outline-blue-800 hover:bg-white hover:text-blue-800 hover:outline-1 hover:outline-blue-800 duration-300 cursor-pointer font-light">
                            ...
                            <div className="flex justify-center items-center relative outline-1 outline-blue-800 grow h-10 md:w-fit w-full">
                                    <input type="text" name="search" placeholder='Search...' className="bg-white text-black w-full h-full px-4 py-2 font-medium focus:outline-0" value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => {if (e.key === "Enter") {nav(`/home/${category}/${search}`)}}} autoComplete="off"/>
                                    <Link to={`/home/${category}/${search}`}>
